...
class TeamAssigner:
    ...
    def get_player_color(self, frame, bbox):
        image = frame[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])] # bbox = x1, y1, x2, y2
        ...
        probs = logits_per_image.softmax(dim=1)  # softmax return a probability

        class_name = classes[probs.argmax(dim = 1)[0]] # argmax return index of highest probability
        return class_name
